...
| 2本指 → スクロール | ✅ |
| 3本指以上 | 追跡・可視化のみ |
| ピンチ・回転のネイティブジェスチャ送出 | ❌ 未実装（[出力側の制約](#出力側の制約)参照） |
...
HIDTouch は接続時にこれを書き込み、**書き込んだ値を読み戻して実際に受理されたか
検証**します（成功を返しつつ無視するパネルがあるため）。
バッファ形式も `[reportID, mode, id]` と `[mode, id]` の両方を試します。
...
ピンチ・回転のネイティブジェスチャは、非公開の `CGEvent` ジェスチャフィールドを
直接叩けば技術的には到達可能です。ただし macOS のバージョン間で互換性の保証が
まったく無いため、本プロジェクトでは意図的に使用していません。
